...
void Task::updateHook()
{
    ...
    if ((_motion_command.read(current_motion_command) == RTT::NewData))
    {
        ...
        {
            // LOG_DEBUG_S<<"locomotion_control::Task::motion_commandCallback: new command
            // received...";
            state = NEW_COMMAND;
...
void Task::sendCommands()
{
    ...
    // for (int j = 0; j<12; j++)
    // {
    //     LOG_DEBUG_S << "Sending:      " << joints_commands[j].position << "\t" <<
    //     joints_commands[j].speed << std::endl;
    // }
}
...
void Task::sendSteeringCommands()
{
    ...
    {
        joints_commands[COMMAND_WHEEL_STEER_FL].position =
            locCtrl.commands[COMMAND_WHEEL_STEER_FL].pos;
    }
    ...
    {
        joints_commands[COMMAND_WHEEL_STEER_FR].position =
            locCtrl.commands[COMMAND_WHEEL_STEER_FR].pos;
    }
    ...
    {
        joints_commands[COMMAND_WHEEL_STEER_CL].position =
            locCtrl.commands[COMMAND_WHEEL_STEER_CL].pos;
    }
    ...
    {
        joints_commands[COMMAND_WHEEL_STEER_CR].position =
            locCtrl.commands[COMMAND_WHEEL_STEER_CR].pos;
    }
    ...
    {
        joints_commands[COMMAND_WHEEL_STEER_BL].position =
            locCtrl.commands[COMMAND_WHEEL_STEER_BL].pos;
    }
    ...
    {
        joints_commands[COMMAND_WHEEL_STEER_BR].position =
            locCtrl.commands[COMMAND_WHEEL_STEER_BR].pos;
    }
...
bool Task::targetReached()
{
    for (unsigned int i = 0; i < joints_readings.size(); i++)
    {
        ...
        // When there are no joint_readings, they are "nan". Thus, all the following if-checks of
        // the position and velocities are false.
        // Therefore the program says the target has been reached even though there is NO
        // information about the real state of the system available.
...
void Task::getSteeringPositionReadings(base::samples::Joints joints_readings,
